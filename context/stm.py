...
async def store_conversation(conversation_id: str,state: str, message: dict[str,Any]) -> dict[str,Any]:
    ...
    # print(existing_data)
    existing_data[state].append(message)

    async with aiofiles.open(file_path, "w") as f:
        await f.write(json.dumps(existing_data, indent=4))
        
    print(f"Storing message for conversation {conversation_id}: {message} with state {state}")
    return {"status": "success", "conversation_id": conversation_id, "state": state, "message": message}
...
async def get_conversation(conversation_id: str,state: str, messages: int | None = None) -> dict[str,Any]:
    ...
    return {"status": "success", "conversation_id": conversation_id, "state": state, "message": existing_data[state][-int(min(len(existing_data[state]),messages)):] if messages else existing_data[state][-1]}
